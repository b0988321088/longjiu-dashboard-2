...
def generate_dragon_five_report():
    ...
    report += (
        f"\n📅 *配息时间轮动（三站无空窗机制）*\n"
        f"站1（07/07 摩根JPM）：FJ33 原持有摩根，基准日07/07，转換申請摩根→安联收益成长（T+2=7/13），可参与安联基准日07/14，预计7/19入帐。\n"
        f"站2（07/14 安联收益成长）：QL18610694/B 原持有安联收益成长，基准日07/14，已转出→M&G入息（T+4=7/15），本期不参与安联配息，但可参与M&G基准日07/17。\n"
        f"站3（07/17 M&G入息）：QL18610694/B 转换中，M&G基准日07/17，T+4=7/15，预计7/29入帐。\n"
        f"月底站（07/29-30）：安联AI收益、贝莱德A10，监控中。\n"
    )
