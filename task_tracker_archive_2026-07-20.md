...
✅ **已自動修正**
- index.html redeploy 註解日期更新：`202607162200` → `202607202200`
- full_monitor.py 簡體字置換：时间→時間、基准→基準、转换→轉換、参与→參與、贝莱德→貝萊德、入帐→入帳、无空窗→無空窗
- t_navigation.py 簡體字置換：国定→國定、导航→導航、输入→輸入、计算→計算、可读→可讀、参与→參與
- decision_buttons.py、decision_utils.py：延后→延後
- market_intel.py：情报→情報、自动→自動
- dashboard_structure_20260709.md 簡體字置換：安联→安聯、贝莱德→貝萊德、触发→觸發、系統/執行/手動/預計→正體
- dashboard_wireframes_20260709.md、Hermes-Intelligence.md 簡體字置換
...
- changelog_2026-07-17.md 真值異常：總資產與保單現值顯示 0 TWD，與 snapshot.json（50,689,930 / 9,876,282）不符
- framework_snapshot_2026-07-11_final.json 內容錯誤：實際為 dashboard.py 內容，龐大單行（17,330 字元），應修正檔名或恢復正確內容
- snapshot.json 日期為 2026-07-18，已兩日未更新（需產生 07/20 snapshot）
- index.html 日期文字顯示 2026-07-20 正確，但資料內容仍基於 07/18 snapshot（需手動更新 snapshot 後重新部署）

🔴 **需要人工介入**
- scripts/ 目錄與流程腳本重建（結構性）
- dashboard.py base64 snapshot 更新與架構改為外部載入（程式碼層級）
- framework_snapshot_2026-07-11_final.json 為 dashboard.py 誤植，需人工確認並修正內容
- CBH轉帳清單_20260713.md 檔案名稱含簡體字「转账清单」→ 應為「轉帳清單」（需人工決定是否更名）
