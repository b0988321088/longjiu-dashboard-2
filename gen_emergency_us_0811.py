# -*- coding: utf-8 -*-
"""美股緊急應變 8/11 21:40 — 從 data/emergency_llm_analysis.json 渲染兩版 HTML
產出：emergency_report_2026-08-11.html (Railway 版) + emergency_taiex_report_2026-08-11.html (GitHub 版)
"""
...
{kpis_html}
{sec_html}
<div class="foot">龍九控股內部報告｜僅供決策參考，非投資建議｜資料時間 {NOW} 台北（美股開盤約 10 分鐘）｜Yahoo Finance 即時 + CNBC + Google News RSS + snapshot.json 穿透數據</div>
</div></body></html>"""
...
def build():
    ...
    kpis = [
        ("道瓊", "54,039.91", "+0.12%", "up"), ("S&P 500", "7,757.40", "+0.06%", "up"),
        ("納斯達克", "26,603.10", "-0.01%", "flat"), ("費城半導體", "12,110.81", "+0.98%", "up"),
        ("台積電 ADR", "420.33", "+0.44%", "up"), ("NVDA", "220.83", "+1.51%", "up"),
        ("VIX", "15.53", "+0.45%", "flat"), ("US30Y", "5.231%", "防禦5.20", "warn"),
        ("WTI", "81.62", "-0.62%", "down"), ("黃金", "4,458.60", "+0.88%", "up"),
    ]
    ...
    for title, paras in secs:
        ...
        if "現金紅線" in body or "低於 85.2" in body:
            body = body.replace(
                "【六、風控檢查】", "【六、風控檢查】", 1)
