...
def _build_insurance_detail(snap: dict, insurance_total: float) -> dict:
    """統一建構保險明細"""
    snap.setdefault("_debug_logs", []).append(f"Insurance Detail Build Start")
    def _extract(v):
    ...
    for _n, _v in _ins_brk.get("policy_a_funds", {}).items():
        ...
        snap["_debug_logs"].append(f"Fund name: {_n}, original label: {_n}") # DEBUG
        _label = _n # Original label
        if "黃金基金" in _n or "黃金" in _n:
            _label = f"黃金投資 ({_n})"
        elif "健康科學基金" in _n or "健康科學" in _n:
            _label = f"健康科學投資 ({_n})"
        snap["_debug_logs"].append(f"Final label: {_label}") # DEBUG
...
def build_html(rows: list[dict], history: dict, snap: dict) -> str:
    ...
    # 從 snapshot holdings 列表動態讀取股價與名稱（無硬編碼）
    _sec_holdings = snap.get("securities", {}).get("holdings", [])
    _price_map = {_h["ticker"]: _h.get("price", 0) for _h in _sec_holdings}
    _name_map = {_h["ticker"]: _h.get("name", "") for _h in _sec_holdings}
    _names.update({k: v for k, v in _name_map.items() if v})
    _total_sec = float(ex.get("securities_market", 0) or 0)
    _list = []
    for _t, _s, _b in _holdings:
        _s = _s or 0
        _price = _price_map.get(_t, 0)
        _mv = _s * _price
