#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""alerts.py — 條件式提醒單一入口（整合 4 支：health_alert_check/reminder_agent/entry_monitor/pnl_rebalance_alert）。

  morning  盤前：健康度紅線（09:10）
  evening  盤後：主動提醒＋進場監控＋獲利超標（20:00）
  （各腳本原本就只在有觸發時輸出；無觸發＝零輸出＝不推播）"""
import argparse
import os
import subprocess
import sys
...
DEFAULT_MODE = 'evening'
MODES = {'morning': [('health_alert_check.py', (), False)], 'evening': [('reminder_agent.py', (), False), ('entry_monitor.py', (), False), ('pnl_rebalance_alert.py', (), False)]}
