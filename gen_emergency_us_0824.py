# -*- coding: utf-8 -*-
"""美股緊急應變 8/24 21:30 — 從 data/emergency_llm_analysis.json 渲染兩版 HTML
產出：emergency_report_2026-08-24.html (Railway 版) + emergency_taiex_report_2026-08-24.html (GitHub 版)
...
{kpis_html}
{sec_html}
<div class="foot">龍九控股內部報告｜僅供決策參考，非投資建議｜資料時間 {NOW} 台北（美股開盤）｜Yahoo Finance chart API 即時 + daily_analysis briefing + us30y_state/FRED DGS30 + snapshot.json 穿透數據</div>
</div></body></html>"""
...
<ul>{lis}</ul>
<div class="alert">⚠️ 美股成長 43.1% 超標 +13.1pp｜台股 7.2% 不足 -16.3pp｜今晚費半 -2.28% 主曝險來源</div></div>"""
...
def build():
    ...
    kpis = [
        ("道瓊 (DIA)", "53,394.31", "+0.22%", "up"), ("S&P 500 (SPY)", "7,657.07", "-0.23%", "down"),
        ("納斯達克 (QQQ)", "26,038.87", "-0.54%", "down"), ("費城半導體 (SOXX)", "11,473.06", "-2.28%", "down"),
        ("台積電 ADR", "411.15", "-1.86%", "down"), ("NVDA", "213.88", "-0.39%", "down"),
        ("AAPL", "310.55", "+0.39%", "up"),
        ("US30Y", "5.23%", "防禦5.20/凍結5.30", "warn"), ("台股加權", "44,762.32", "-1.02%", "down"),
    ]
