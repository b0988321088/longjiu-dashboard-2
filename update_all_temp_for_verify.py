...
96|            ('penetration_monitor.py', [sys.executable, str(project_dir/'penetration_monitor.py')]),
97|            # ('daily_deploy.py', [sys.executable, str(project_dir/'daily_deploy.py')]),
98|        ]
99|
100|        for name, cmd in steps:
101|            print(f"Executing {name}...")
102|            subprocess.run(cmd, check=True)
103|
104|        sec_total = args.get("sec", 0) # Use updated securities total from args
105|        if sec_total:
106|            print("\n=== 產出校驗 ===")
107|            sec_str = f"{sec_total:,}"
108|            files = [("日報", BASE/f"daily_report_v2_{TODAY}.html"),
109|                     ("儀表板", BASE/"index.html"),
110|                     ("差異分析", BASE/f"asset_diff_{TODAY}.html")]
111|            all_ok = True
112|            for label, fp in files:
113|                html = fp.read_text(encoding="utf-8", errors="ignore")
114|                if sec_str in html:
115|                    print(f"  ✅ {label} 證券值一致 ({sec_str})")
116|                else:
117|                    print(f"  ❌ {label} 證券值不符（預期 {sec_str}）")
118|                    all_ok = False
119|            if all_ok:
120|                print("  ✅ 三份產出一致")
121|            else:
