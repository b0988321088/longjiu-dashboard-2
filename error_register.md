...
## INCIDENT cash_floor_two_calibers_unlabeled (data_consistency)

- 首次發生: 2026-09-25（使用者問「我該如何因應」時，同日兩份產出互相矛盾）

- 現象: 同一天，`rebalance_eval_2026-09-25.html`／`rebalance_summary_2026-09-25.md`／日報寫「現金底線 862,409 vs 底線 1,200,000 → 未達標（缺口 337,591），本表唯一紅字」，而 `rebalance_dashboard_2026-09-25.html` 與 `build_dashboard.py` 的風險表顯示「現金底線 86萬 ≥70萬 🟢 安全」。

- 根因: `snapshot.thresholds_2026_0915.現金_twd` 是三欄位 SoT（生活底線 700,000／追繳緩衝 500,000／合計底線 1,200,000），但消費端各取一半且都不標口徑 —— dashboard 只取生活底線、緊急應變／再平衡評估只取合計底線 → 同一 SoT 的兩個子項被當成同一件事的兩種答案。`debt_restructure_tracker.py` 已做對（現金 < 合計底線 → 🟡 追繳緩衝不足 → 配息導流補足、不新增買入），dashboard 未跟上。

- 影響: 使用者同日看到「🟢 安全」與「🔴 缺 33.8 萬」，無法判斷該不該動用乾粉；且本案 agent 第一時間把它誤診為「LLM 幻覺門檻」，錯誤診斷本身就是二次傷害（正確做法是先讀 SoT 全文欄位）。

- 修法（待核准）: `build_rebalance_dashboard.py`（md 第六節 ＋ HTML 風險紅線兩處）與 `build_dashboard.py` 的現金列改為雙口徑標示 —— 生活底線（≥700,000）＝ ✅／乾粉；合計底線（含追繳緩衝 500,000）＝ 🟡 缺口金額＋導流動作；全部讀 `thresholds_2026_0915.現金_twd`，不新增常數。

- 附帶待查: snapshot `cash_total` 861,818 與穿透桶「現金/安全網」862,409 差 591 元，需確認差項來源（不影響本次判定）。

- 教訓: ①同一 SoT 的多子項欄位，任何顯示端都必須標明取的是哪一個子項 —— 口徑標籤與數字同等重要 ②「同一件事在兩張卡上給出相反判定」要當缺陷處理，不能因為兩邊都算得出來就放過 ③遇到資料矛盾先讀 SoT 全文，不要先歸因模型幻覺。

- 殘留（2026-09-25 修法落地後）：①緊急應變 LLM 章節（`data/emergency_llm_analysis.json` 六、風控檢查）與日報描述仍以「底線 1,200,000」單一口徑敘述（判定正確但未標口徑）②`buffett_cto_analyzer.py` 的敘述句仍寫生活底線措辭（≥70萬）。兩者皆為文案層、不影響判定，列低優先清理。
- 狀態: ✅ 已修（2026-09-25，commit 70144027 tree c1405e93／CIO-DeepSeek-Flash APPROVE）；殘留項列低優先待辦

## INCIDENT narrative_gate_engine_offset_caliber (false_positive)
- 首次發生: 2026-09-25 22:40（收工檢查 cron d6282d33346a 失敗回報）
- 現象: 閉環稽核第 13 類 ❌「內文數字對不上 snapshot（1 處）」—— `emergency_llm_analysis.json` 內文「債券 +5pp ∉ 合法值 [2.0, 3.7, 4.7]」
- 根因: 內文引用的是 **DAA 引擎戰術偏移**（macro_regime targetAllocation：債券 目標 25 → 燈號偏移後 30 ＝ +5pp），而 `check_narrative_numbers.build_allowed` 的 債券 gap 集合只收「現況缺口」派生（gaps 3.7／債券及安全現金 2.0／佔投資部位 4.7）→ 合法的引擎口徑被當成錯數字。觸發源＝9/25 紅線期因應卡以「引擎建議金額（…債券 +5pp…）僅列觀察」措辭落成 pending，21:30 緊急應變報告沿用該措辭。
- 修法: `build_allowed` 新增 `_engine_offsets()`：讀最新 `macro_regime_*.json` 的 `targetAllocation.rows`，以「燈號偏移後 − 目標」補入對應桶的 gap 集合（台股／美股／防守／債券；底線制與衛星列自動跳過），不寫死任何數字、不放寬其他檢查
- 驗證: 自測 12/12（正向：+5pp／-8pp／+3.7pp／+4.7pp／+5.0pp；負向仍擋下：+9pp／+12pp／科技 17.5%／現金 999,999）＋守門實跑 ✅；allowed 債券 gap=[2.0, 3.7, 4.7, 5.0]、美股=[-8.0, 9.5]
- 教訓: ①守門的合法值集合必須涵蓋「內文會自然引用的所有派生口徑」—— 現況缺口 ≠ 引擎戰術偏移，兩者都是真值 ②要把數字寫進會被 LLM 引用的文字（決策卡／裁示／pending）時，先問「這數字屬於哪個口徑、守門認得嗎」 ③同型已第三次（INC-245 科技分母、INC-244 現金派生、本次引擎偏移）→ 一律修集合、不改內文、不放寬
- 狀態: ✅ 已修（2026-09-25）
- 根治（2026-09-25，使用者要求「修復並避免再發生」）：①合法值集合改為**一次補齊引擎三種口徑**（戰術偏移 ±、偏移後目標值、建議金額），不再逐次補單一值（函式 `_engine_calibers`）②新增 **repo 內回歸自測** `check_narrative_numbers_selftest.py`（19 案例：引擎/穿透/現金派生/不該掃的型態），並讓收工稽核第 13 類**先跑自測再實掃**——守門被改壞與合法集合落後不再互相掩護 ③❌ 訊息直接印出「在哪裡補來源、要補哪個案例」的修復指引 ④**源頭治理**：兩支緊急應變 cron prompt 內殘留的舊「階段目標」與舊核貸時程數字（美股30/台股23.5/防守19/債券13/現金14.5、8/15 撥款→清償800萬→400萬分批）已全數刪除，改為「一律現讀 snapshot.penetration」＋新增步驟「存檔前跑 check_narrative_numbers.py，非綠不進產報」——過去 LLM 就是照 prompt 舊數字寫，才反覆觸發本類 ❌
- 同型紀錄：4 次都在 3 天內（INC-245 科技分母 → 現金派生 → 引擎偏移 → 偏移後目標值/建議金額），此後判準＝「集合落後就補集合＋補案例，並回頭檢查 prompt 是否在餵舊數字」

## INCIDENT narrative_gate_dir_and_coverage (gate_hardening)
- 首次發生: 2026-09-25（對 commit 8f8f03f3 做唯讀程式審查時，CIO 提出 5 項殘留風險；使用者核准 A+B 批）
- 現象: 8f8f03f3 修掉第 13 類誤報，但審查發現 5 項殘留：①引擎偏移把 ±delta 都塞進同一個 gap 集合 →「幅度對、方向反」的敘述會被放行 ②pct 集合同時收現況值與引擎目標值（語意混用）③缺檔／壞檔靜默 return（無法區分內文錯或引擎檔壞）④選檔用 sorted(glob)[-1] 字典序 ⑤避險衛星列未納入覆蓋
- 根因: gap 集合混雜兩個不同意義的有號量 —— 現況缺口（實際−目標）與引擎戰術偏移（偏移後目標−目標），同一標籤可能符號相反（債券現況低於目標＝負缺口，但引擎要加碼＝正偏移）→ 單一集合表達不了，於是靠「±都收」妥協，代價是方向寫反不會被抓
- 修法（A+B 批，2026-09-25）: ①引擎偏移改存獨立 `off` 集合（有號、不翻符號）；pp 比對依內文語境詞分流 —— 超標/超配/低配→現況缺口，戰術偏移/偏移後→引擎偏移，判不出→兩集合聯集（＝修前行為）；寫了正負號必須同號，沒寫號（「美股 8pp」＝減碼幅度）只比幅度 ②目標值本身也補入 pct ③缺檔／壞檔／rows 型別不符一律出聲到 stderr ④選檔改為只認 `macro_regime_YYYY-MM-DD.json` 並取最新日期（備份檔不再可能蓋掉日期檔）⑤新增避險衛星覆蓋（標籤 `避險衛星`／`衛星`），刻意不註冊 黃金／石油（會與商品報價「黃金 2,400」這種千分位數字碰撞）⑥本函式新建而補不到值的標籤一律移除並警示（空集合在 scan_text 內＝跳過比對＝靜默放行＝假陰性）
- 語境詞只收單義者: 缺口／不足／建議／加碼／減碼 一律不收 —— 實測語料顯示模型會拿它們講「建議幅度」而非現況缺口（「缺口：台股 -2.2pp」是現況，「建議收回 9.5pp」是建議），硬套會製造假陽性；且取語境詞時以標點切段，避免鄰句污染（「美股超標 9.5pp、債券 +5pp」的後者不得被前者的「超標」帶走）
- 驗證: 自測 19→29 案例全過（新增 5 組口徑分流＋5 組衛星／不掃案例，其中 4 組負向）；**語料回歸 168 檔**（全部 buffett_cto_report_*.md＋emergency_llm_analysis.json＋所有 cto 快取）修前 206 命中 vs 修後 206 命中，**新增 0、消失 0 → 零新增誤報**；守門實跑 ✅；退化路徑 7 種全部出聲（含「只有備份檔」不再被誤用、空集合標籤被移除並警示）
- 教訓: ①同一個集合放兩種語意的量，遲早要用「放寬」妥協 → 該拆就拆，拆完語境詞只收單義者 ②守門的失敗方向要選「吵」不要選「靜」：缺檔／壞檔出聲（假陽性可查），空集合移除（假陰性不可見）③新增掃描標籤前先問「這個詞會不會跟別的數字型態碰撞」（黃金 2,400/oz）④寫入端與讀取端必須同批 patch —— 中間態必然自測失敗，不是「先紅後綠」而是無效狀態
- 殘留（C 批，刻意不動程式）: pct 集合同時收現況值與引擎目標值（例：防守型配息 32.0），要區分須綁語境詞但收益低（讀者看章節即知是目標）→ 列 known limitation；另有兩項低優先：衛星個別標籤（黃金／石油）未註冊、引擎檔口徑若落後當日仍會被當合法值（選檔已限定日期命名，但沒有「日期必須 ≥ 今日」檢查）
- 空集合盲區（獨立複審必修 4，2026-09-25 明列）: `allowed` 每個標籤只補「它有來源」的口徑，`scan_text` 對空集合一律跳過比對 → 仍有 7 處不掃的盲區：現金 gap/off、科技 off、非科技 gap/off、金融 gap/off/twd、資訊科技 gap/off/twd、衛星 gap、避險衛星 gap（例：寫「金融 1,234,567」或「現金 5pp」不會被檢查）。皆為既有設計 —— 本批只新增標籤、未新增盲區（新增的「移除＋警示」僅涵蓋 `_engine_calibers` 新建的標籤；引擎偏移口徑為空的標籤會退回兩集合聯集，不會靜默跳過）。已在 `check_narrative_numbers_selftest.py` 以兩條案例固定行為，任何「改成會掃」的變動都必須是刻意的
- 複審（2026-09-25，獨立審查子代理，tree c2d48006）：第一次送審 **REJECT**，抓到兩條本批自測漏掉的迴歸 —— ①**新假陽性**：鄰句無標點時語境詞會跨句污染（「美股超標 9pp 但債券 +5pp」把債券的引擎值判成現況缺口；自測原案例寫成「9.5pp、」帶小數點與頓號，而小數點剛好是切段字元才意外過關）②**新假陰性**：語境詞解析為引擎偏移、但該標籤 off 集合為空時（例 科技），原本的 `continue` 讓舊版會擋的數字靜默放行。修法：切段依據加入 `\d`（任何數字都切）、口徑集合為空時退回兩集合聯集、既有標籤的引擎列全非數字時也出聲（原本只有「新建標籤」分支會警示，既有標籤靜默）。兩條已加成自測負向案例（29→31）
- 教訓（複審補）: ①驗證自己的修正不能只跑「自己設計的案例」——案例的標點與小數點會掩蓋缺陷，必須涵蓋無標點／整數的變形 ②`if not <集合>: continue` 這種守衛式跳過，改成多集合後等於憑空新增假陰性通道，必須改成「退回上一版的比對範圍」 ③用 execute_code 驗證時 kernel 會快取已 import 的模組，改檔後必須用獨立程序重驗，否則拿到的是舊物件（本案實際踩到，一度誤判修正無效）
- 殘留 #3 結案（2026-09-25 第二顆 commit）: 複審指出「引擎列整列缺列（列名被改名）、或僅部分欄位解析失敗（例 target 還在、燈號偏移後 變字串）時仍靜默」——與本檔存在目的同型（引擎口徑悄悄消失，症狀只顯示成內文數字對不上、指不到根因，且半套補入比全錯更難查）。修法：`_engine_calibers` 逐列記錄「出現過哪些 ENGINE_ROWS 期望列名」與「哪些欄位解析失敗（None 或非數字）」，補完後對 stderr 出聲；並把這兩類＋「完整檔不得出聲」寫成自測的警示案例（33→38 案例）。驗證：真檔實跑 stderr 全空（零雜訊，否則每日稽核會被淹掉）、5 種合成情境全數正確（含「完整檔不得出聲」負向項）、語料 168 檔與前一版逐筆比對新增 0 消失 0（總命中 212 筆；原文寫「206/206」未標口徑，經審查者實測提醒後修正）
- 複審補（2026-09-25，同一 commit 內修正）: 審查者用**變異測試**抓到自測的假信心 —— 上述「部分欄位型別錯」「欄位被改名」兩案例原本只放單列 payload，光是缺列就讓 `_missing` 分支先出聲、案例即判定通過；把 `if _partial:` 整段刪掉，自測照樣印 38/38、exit 0（＝該生產路徑零回歸保護）。修法：警示案例改為「完整 5 列＋單一欄位突變」，並以變異測試雙向驗證（刪 `_partial` → FAIL、刪 `_missing` → FAIL）。**教訓：要測「某個分支會出聲」，必須讓其他分支不會出聲（完整輸入＋單一突變）；否則測到的是別條分支。** 另依審查者提醒，在 ENGINE_ROWS 註解記下引擎檔那列刻意非數字的「台幣現金+短定存」——未來若納入本表需同步加白名單，否則會每天誤響
- 收尾批（2026-09-26，第三顆 commit；使用者指示「一次做完再送審」）: ①補齊兩條舊警示分支的回歸案例 —— 『新建標籤補不到值』的關鍵不是訊息而是**行為**（該標籤必須被移出 allowed，否則空集合＝靜默放行），故自測改為斷言 allowed 內不得存在該標籤；把 `del allowed[lab]` 改成 `pass` 現在會讓自測轉紅（39/40），修前不會 ②新增引擎檔齡警示（最新檔 ≥3 天前才出聲；今日檔在引擎跑之前本就不存在，1–2 天屬正常空窗，門檻 3 天才不會天天誤響）③收掉與 `_partial` 完全重複的「該列三欄位全非數字」彙總訊息（同情境必然逐欄位各報一筆；獨立複審已判定「訊號冗餘、無資訊損失」）。變異測試 6/6：clean 40/40 rc=0；del_partial 38/40、del_missing 39/40、del_removal 39/40、del_staleness 39/40、del_seen_add 39/40 全部 rc=1。驗證：最新引擎檔與真實入口 stderr 全空；allowed 對上一版逐鍵逐值相同（本批純 stderr）；語料 168 檔 206/206 去重命中（原始 212）新增 0 消失 0
- 收尾批二（2026-09-26，第四顆 commit；上一顆獨立複審的非阻擋後續，一次補完）: ①檔名日期不合法（例 `macro_regime_2026-13-45.json`）原本會讓 `date.fromisoformat` 抛 ValueError → 被最外層 except 吞掉 → **整個函式放棄**，連合法檔的引擎口徑一起丟（引擎值全變成假陽性）—— 此為上一顆新引入的失敗模式（舊版是同情境靜默用錯檔）；改為逐檔驗證、跳過壞檔並指名 ②未來日期檔（今天之後）會讓 `sorted()[-1]` 勝出成權威口徑，且負的 age 讓檔齡警示永不觸發＝完全靜默；補 `_age < 0` 出聲（仍照用，拒用會讓合法值全變假陽性）③檔齡門檻 3→2 天：引擎每日產（實測 36 檔連續無缺口、七個星期全到齊），正常空窗上限就是 1 天，age>=2 即代表漏跑一次，可早一天發現且無誤響 ④補 `if not _files:`（glob 回空）的專屬回歸案例 —— 複審指出這是全函式唯一沒有案例的警示分支 ⑤ENGINE_ROWS 註解更正：非註冊列實為三列（另有 石油/能源(避險衛星)、黃金/實質資產(衛星)）⑥`_partial` 與標籤移除訊息補上引擎檔名（原本只有 `_missing` 有）。自測 40→44；變異測試 9/9：clean 44/44 rc=0；del_partial 42/44；del_age_ge2 42/44；del_missing／del_removal／del_seen_add／del_nofiles_warn／del_baddate_skip／del_future_warn 各 43/44 —— 全部 rc=1 且 FAIL 清單恰為對應案例。驗證：最新引擎檔與真實入口 stderr 全空；allowed 對 bc16ed49 逐鍵逐值相同（純 stderr 批）；語料 168 檔 206/206 新增 0 消失 0
- 狀態: ✅ A+B 批已修＋複審四項必修已補＋殘留 #3 結案＋收尾批＋收尾批二（2026-09-26；四顆 commit）；C 批僅剩 pct 語意混用列 known limitation
