...
## 2026-09-21（INC-233）儀表板 3 個月安全線動態更新靜默失效（鍵名數字轉置 458,343）

- **症狀**：`index.html`／`dashboard_v2.html` 的「3 個月安全線」一直是硬編碼值，`build_dashboard.py` 的動態更新從未生效；目前無可見症狀，只因硬編碼值恰好等於月支出×3。
- **發現管道**：非本輪改動引起，是 2026-09-21 日報「每月固定支出分層」程式 commit `c7902f93` 送 CIO 審查時，審查者順手盤點讀者時查出的範圍外既有缺陷（reviewer 標記為 residual risk，非阻擋）。
- **根因**：`build_dashboard.py:248` 取用 `rep["458,343"]`（鍵名本身就是數字字串，且數字轉置，應為 488,343），該鍵在任何產出 JSON 中都不存在 → 取不到值就靜默走預設／硬編碼分支，`check_dashboard_stale.py:37` 也把 458,343 列為已知舊值，於是「壞掉」與「正常」在畫面上長得一樣。
- **同批殘留風險（低）**：snapshot 同時存有頂層 `monthly_expense_cash`／`monthly_expense_accrual` 與 `monthly_fixed_expense.分層.*` 兩份同值資料，目前只有頂層被讀取，分層子鍵為無讀者副本 → 未來只改一邊會靜默漂移。建議擇一為單一寫入源，或在閉環稽核加一致性檢查（兩份同值比對）。
- **狀態**：已登記，尚未修（本輪範圍外）。修法建議：安全線改由 `monthly_expense * 3` 動態計算（或讀正確鍵 488,343），並在 `check_dashboard_stale.py` 移除 458,343 舊值白名單。
- **教訓**：**「鍵名用數字字串」是靜默失效的溫床** —— 打錯一個數字不會拋錯、只會取不到值；以及報表數字應一律由 canonical 欄位相乘得出，不要出現「恰好等於正確答案」的硬編碼，否則缺陷會長期隱形。
