...
（⛔ 舊文「500 萬 MMF＝10 月標案預備金」已作廢 9/11；押標金 240 萬來源延 9 月底評估）
"""
...
B(s, [
    ...
    '     貝萊德B11 強化穩定月配息 ' + f"{gk.get('貝萊德智慧數據收益成長B11-美元-強化穩定月配息',0):,.0f}".replace(',',',') + ' 元（原 MMF 500 萬贖回款轉申購；⛔ 原「500萬標案預備金」已作廢 9/11）',
...
B(s, [
    f'🔴 保單借貸 {pl/10000:,.0f} 萬 @4%+ → 整池質押後 540 萬@2.77% 償還',
    '     （還安聯 300 萬；餘 240 萬標案押標金），利息成本 4.2% → 2.77%',
    '',
...
B(s, [
    '🔴 現況（9/11／9/12 定案）：',
    '     貝萊德 B11 500 萬（原 MMF 贖回款）＝質押擔保池成員，不可動用；10 月押標金來源延 9 月底評估',
    f'     台股穿透 {p.get("台股市值型成長",0):.1f}% → 偏低，但觀望期凍結新增',
    '',
    '✅ 不動：美股（匯率回吐風險但長線持有）、安聯、FL65、債券梯',
    '',
    '💡 觀望至 9/11 CPI / 9/16 FOMC：',
    '     升息確定 → ⛔ 原「500 萬 MMF 轉還債安全墊」已作廢（MMF 已轉 B11）→ 改由整池質押 540 萬（~9/25 撥款）清償高息負債',
...
B(s, [
    ...
    '     質押擔保池 B11 500 萬（原 MMF）✅ 00983D 短債抗跌 ✅ 質押新增凍結（≥5.30 線）✅',
    '',
    '🟡 情境B：高原（利率不動，最可能）',
...
B(s, [
    ...
    '     升息確定 → ⛔ 原「500 萬 MMF 轉還債安全墊（不換匯不買債）」已作廢；現行＝整池質押 540 萬@2.77% 清償 500 萬高息負債',
    '',
    '📅 9/25：洲際W 1,312 萬到期 → 轉貸定案（築巢 2.185% 優先 / 國泰備案 ≤2.5%）',
    '',
    '📅 10 月：環保標案結果 →',
    '     沒標到 → 重評估匯率/曝險/ladder 時機（或直接還債）',
    '     標到 → 履約保證函優先；⛔ 原「500 萬 MMF 作為營運預備金」已作廢（押標金來源延 9 月底評估）',
    '',
...
B(s, [
    ...
    '      升息確定 → ⛔ 原「500 萬 MMF 轉還債安全墊」已作廢（MMF 已轉 B11）；現行＝整池質押 540 萬@2.77% 清償 500 萬高息負債',
    '      降息/持平 → 10 月標案結果後再評估 ladder',
    '',
    '4️⃣ 【龍九盯】持續 → 雷達 pending：凍結台股/輪動至 9/11 前不加碼',
    '      月配息覆蓋生活費自動入帳，盈餘依序償債（4%+ 優先）',
    '',
    '5️⃣ 【建明】10 月 → 標案結果出爐（⚠️ 押標金 240 萬來源延 9 月底評估；原記「500 萬 MMF 預備金已就位」⛔ 已作廢）',
...
B(s, [
    ...
    '',
    '🏠 債務 ~2.6%：比市場低 4pp+，整池質押 540 萬@2.77% 再降成本',
    '',
    f'📈 台股 {TAIEX:,.0f}（{ASOF} 收盤，單日 {CHG1D:+.2f}%）→ 無系統風險，不恐慌不追高',
    '',
    '🎯 9/11／9/12 定案框架：整池質押 540 萬@2.77%（1,200 萬×4.5 成）→ 清償 500 萬高息負債；',
