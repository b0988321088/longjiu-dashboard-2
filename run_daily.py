...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  h2 {{ font-size: 18px; font-weight: 800; color: var(--text-primary); margin: 16px 0 10px; border-bottom: 2px solid var(--accent-blue); padding-bottom: 6px; }}
  h3 {{ font-size: 15px; font-weight: 800; color: var(--accent-blue); margin: 10px 0 6px; }}
  .label {{ font-size: 11px; color: var(--text-muted); margin-bottom: 4px; letter-spacing: 0.5px; text-transform: uppercase; }}
  .text-lead {{ color: var(--text-body); margin: 6px 0; font-size: 15px; line-height: 1.6; }}
  .num {{ font-family: var(--font-mono); font-weight: 800; color: var(--accent-blue) !important; }}
  .num-green {{ font-weight: 800; color: var(--accent-green) !important; }}
  .num-red {{ font-weight: 800; color: var(--accent-red) !important; }}
  .tag-ok {{ display:inline-block; background:rgba(52,211,153,0.15); color:var(--accent-green); border:1px solid rgba(52,211,153,0.3); padding:2px 8px; border-radius:4px; font-size:12px; font-weight:700; }}
  .tag-warn {{ display:inline-block; background:rgba(251,191,36,0.15); color:var(--accent-amber); border:1px solid rgba(251,191,36,0.3); padding:2px 8px; border-radius:4px; font-size:12px; font-weight:700; }}
  .tag-p0 {{ display:inline-block; background:rgba(248,113,113,0.15); color:var(--accent-red); border:1px solid rgba(248,113,113,0.3); padding:2px 8px; border-radius:4px; font-size:12px; font-weight:700; }}
  .table-wrap {{ overflow-x: auto; margin: 8px 0; }}
  table {{ width: 100%; border-collapse: collapse; background: transparent; font-size: 14px; }}
  thead th {{ background: #162032 !important; color: #fff !important; border-bottom: 2px solid #1e293b !important; padding: 10px 8px !important; font-size: 13px !important; text-align: left; font-weight: 700; }}
  tbody td {{ color: var(--text-body) !important; border-bottom: 1px solid #1e293b !important; padding: 12px 8px !important; font-size: 14px !important; vertical-align: top; background: #0d131d !important; }}
  .table-wrap {{ overflow-x: auto; margin: 8px 0; }}
  table {{
    width: 100%;
