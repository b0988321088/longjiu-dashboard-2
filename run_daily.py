...
def _inject_dashboard(html: str, tv: dict, intel_signals: dict | None = None) -> str:
    ...
    html = html.replace("__PASSIVE_NOTE__", f"房租應收 80,100（1樓24,000+23樓21,000+洲際W33,000+管理費2,100），配息保守預估 {_div_exp:,}/月，台電薪水 {_salary_exp:,}；當月實際已收 {_div_actual + _rent_got2 + _salary_got2:,}（薪水{_salary_got2:,}+配息{_div_actual:,}+房租{_rent_got2:,}）；口徑速記：月支出 162,781（v4 定版）｜配息 {_div_actual:,}（實收=常態）/100,000（保守）｜被動保守 180,100（配息 100,000 + 房租 80,100）")
    # 覆蓋率（常態月收 / 月支出）
    ...
    html = html.replace("__POLICY_A_VAL__", fmt(tv.get("allianz_a_current_value", tv.get("allianz_a", tv.get("allianz_policy_a_value", 0)))))
    html = html.replace("__POLICY_B_VAL__", fmt(tv.get("allianz_b_current_value", tv.get("allianz_b", tv.get("allianz_policy_b_value", 0)))))
