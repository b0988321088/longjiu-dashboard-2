...
{_station_rows}
    ...
    cio_content = []
    cio_content.append('<p><strong>🧑‍💻 CIO 觀點</strong></p>')
    if cio_content_html:
        ...
        try:
            ...
            if _cio_cache.exists():
                ...
                # 2026-09-13 INC-168：原 prompt 只給台股/美股/防守 → LLM 自行推算「現金 25.1%」（真值 3.3%）
                _cio_pen_bond = tv.get("penetration", {}).get("actual_pct", {}).get("債券", 0)
                _cio_pen_cash = tv.get("penetration", {}).get("actual_pct", {}).get("現金/安全網", 0)
                ...
                try:
                    _rsig0 = json.loads((BASE / "radar_state.json").read_text(encoding="utf-8")).get("signals", {}) or {}
                    _sig_txt = "".join(f"{_k}{(_v or {}).get('color','⚪')}" for _k, _v in _rsig0.items())
                except Exception:
                    _sig_txt = "（雷達讀取失敗）"
                _cio_prompt = (
        ...
        if not _cio_llm:
            ...
            try:
                ...
                cio_content.append('<span style="display:block">• 本日市場無重大異常。</span>')
                cio_content.append('<span style="display:block">• 資產配置持續檢視，注意防禦型補碼時機。</span>')
            cio_content.append('<span style="display:block">• 流動性管理穩定，補庫警示已處理。</span>')
    cio_content_html = '\n'.join(cio_content)
...
def _inject_market_intel(html: str, tv: dict, signals: dict, llm_emergency: str = "") -> str:
    ...
    if not buf_content:
        ...
        buf_content.append('<p><strong>🧓 巴菲特式思考</strong></p>')
        buf_content = []
        def _format_line_with_numbers(line):
            import re as _nm
            _b = _nm.sub(r'([0-9,]{3,}\\.?[0-9]*|[+-]?[0-9.]+%|[+-]?[0-9,]+億)', r'<strong style="color:#c2410c">\1</strong>', line)
        ...
        if not buf_content:
            buf_content_lines = []
            buf_content_lines.append('<p><strong>🧓 巴菲特式思考</strong></p>')
            ...
            if diff_bullets:
                buf_content_lines.append('<p><strong>📋 昨日差異帶來的行動啟示</strong></p>')
    ...
    if not cto_content:
        cto_content_lines = []
        cto_content_lines.append('<p><strong>🤖 CTO 技術視角</strong></p>')
    ...
    try:
        ...
        # 2026-08-24：tv 缺 snapshot 欄位 → 補值（防守合併凍結判斷 + 衛星動態）
        _need_rebuild = False
        if "defensive_combined_metric" not in tv or not tv.get("defensive_combined_metric"):
            _need_rebuild = True
        if "rotation_recommendation" not in tv or not tv.get("rotation_recommendation"):
            _need_rebuild = True
        if _need_rebuild:
            try:
                _snap_x = json.loads((Path(__file__).resolve().parent / "snapshot.json").read_text(encoding="utf-8"))
                tv["defensive_combined_metric"] = _snap_x.get("defensive_combined_metric", {})
                tv["rotation_recommendation"] = _snap_x.get("rotation_recommendation", {})
                _tbl = build_table(tv, _us30y)
            except Exception:
                pass
        _rows_html = "".join(
...
# 2026-09-12：`_inject_dashboard()`（921 行死碼，全庫零呼叫；線上儀表板注入由 build_dashboard.py 負責）
# 已於本次稽核刪除 — 詳見 INC-156；原始內容可由 git tag `pre-cleanup-20260912` 還原。
