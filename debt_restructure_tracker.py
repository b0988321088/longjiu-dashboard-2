...
def main():
    ...
    if dp.get("status", "").startswith("兩層槓桿修正版") or dp.get("status", "").startswith("8/20 定案"):
        ...
        print("  ⛔ 已作廢（9/11／9/12 取代）— 8/20 定案：富達600萬＋MMF600萬(8/21買) → PI後質押300-400萬@2.77%")
    ...
    if not can_do_lombard:
        ...
        n += 1
    if today < date(2026, 8, 15):
        print(f"  {n}. 🔵 9/11／9/12 定案：整池質押 540萬@2.77%（富達600＋聯博100＋B11 500＝1,200萬×4.5成）→ 9/25 撥款後清償 500萬高息負債（保單400＋券商100）；⛔ 原『買富達600+MMF600萬→MMF轉標案預備金』已作廢")
        n += 1
